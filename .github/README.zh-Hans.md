...
改名/删除……；`connect_and_label`、`place_pwr_flag` 等连通性宏会展开为核心 op）
写入 KiCad 原理图。写入是精准且幂等的（确定性 UUIDv5），由纯 Python 连通性校验器
**加上写入前后的 net diff** 把关，并要求显式 `--apply`（默认为 dry run）。
`akcli undo` 可回退上一次写入。
...
- **Claude Code** — 安装随附的插件（见下方），即可获得 `/altium-kicad:circuit-review`、
  ...
  `design-calc`（`akcli calc` 的 60 个附标准引用的工程计算器）。
...
图纸**）、net 推断、ERC/power/BOM/diff/pinmap/intent 检查、KiCad 写入/绘制（17 种 op + 9 种宏，
含 delete/move/rename 与多单元放置、net-diff 安全护栏，输出经 KiCad 自身 netlister 验证）、
内嵌符号库 relink，以及附订购 CSV 导出的 JLCPCB / LCSC 元件搜索。完整里程碑规划
（v0.2 → v1.0，各里程碑附验收条件）见 **[ROADMAP.md](../ROADMAP.md)**。重点待开发项目：

- Altium `.PcbDoc` 其余**二进制**段（填充/区域/文本/多边形）——ASCII 段与二进制铜箔
  （焊盘/走线/过孔/圆弧）已可读取。
