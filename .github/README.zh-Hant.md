...
- **附標準引用的計算器。** `akcli calc` 回答 56 種設計計算（E 系列、IPC-2221、貫孔寄生參數、
...
`akcli calc` 內建 **56 個離線計算器**——E 系列取值與電阻組合搜尋（IEC 60063）、分壓器、
LM317／FB 穩壓最壞情況、IPC-2221 走線寬度與電氣間距、貫孔寄生參數、熔斷電流、AWG 線規、
微帶線／帶狀線阻抗、RF 衰減器、buck／boost 功率級、NE555、運算放大器增益、I²C 上拉、
晶振負載電容、熱設計、電池壽命、電阻標示碼、電偶腐蝕相容性。**每筆結果都印出正式引用來源**
...
`place_component` op-list。`tools/calc-view/` 內附本地網頁 UI（實體樣式 SVG
圖示、可分享連結）。
...
- **Claude Code** — 安裝隨附的外掛（見下方），即可取得 `/altium-kicad:circuit-review`、
  ...
  `design-calc`（`akcli calc` 的 56 個附標準引用的工程計算器）。
