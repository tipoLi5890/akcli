...
把關（check／intent／contract／library／sch-pcb／fab／order／git），並寫出一份綁定輸入雜湊、git
版本與各項把關發現的 manifest。當 KiCad GUI 開著檔案時，KiCad 寫入會拒絕並回報
`TARGET_LOCKED`（可用 `--allow-open` 覆寫，之後在 KiCad 執行 File→Revert）。完整指南見
...
目前已提供（v0.7.x）：KiCad 寫入／繪製（18 種 op + 9 種巨集，含階層 `add_sheet`、net-diff
安全護欄、`new`／多層 `undo`，輸出經 KiCad 自身 netlister 仲裁）、ERC/power/BOM/diff/pinmap/
**intent**／**contract** 檢查（含 waiver 與 SARIF）、原理圖 ↔ PCB **`verify`**、專案
**`library`** 工作區（audit／repair／import-altium——Altium `.PcbLib` footprint 匯入 + 深度
`.kicad_pcb` 讀取）、版本化的 **`fab`** profile，以及 **`release preflight`** 把關
