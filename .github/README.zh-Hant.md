...
改名／刪除……；`connect_and_label`、`place_pwr_flag` 等連通性巨集會展開為核心 op）
寫出 KiCad 電路圖。寫入是精準且冪等的（確定性 UUIDv5），由純 Python 的連通性驗證器
**加上寫入前後的 net diff** 把關，並需要明確的 `--apply`（預設為 dry run）。
`akcli undo` 可回復上一次寫入。
...
- **Claude Code** — 安裝隨附的外掛（見下方），即可取得 `/altium-kicad:circuit-review`、
  `circuit-pinmap`、`circuit-draw`、`circuit-diff` 指令與八個 skills：`circuit-design`（讀取／分析／
  ...
  `parts-sourcing`（JLC／LCSC 零件選型）、`jlcpcb-capabilities`（JLCPCB 製程能力參考）、
  `design-calc`（`akcli calc` 的 60 個附標準引用的工程計算器）。
- **Codex** — 安裝隨附的外掛（見下方）：內含全部八個 skills 與 session hook；或把 skills 資料夾放進
...
sheet**）、net 推論、ERC/power/BOM/diff/pinmap/intent 檢查、KiCad 寫入／繪製（17 種 op + 9 種巨集，
含 delete／move／rename 與多單元放置、net-diff 安全護欄，輸出經 KiCad 自身 netlister 驗證）、
內嵌符號庫 relink，以及附訂購 CSV 匯出的 JLCPCB／LCSC 零件搜尋。完整里程碑規劃
（v0.2 → v1.0，各里程碑附驗收條件）見 **[ROADMAP.md](../ROADMAP.md)**。重點待開發項目：

- Altium `.PcbDoc` 其餘**二進位**區段（fill/region/text/polygon）——ASCII 區段與二進位銅箔
  （pad/track/via/arc）已可讀取。
